...
exports.getListPage = async (req, res) => {
    try {
        ...
        const [pageRows] = await db.query(
            `SELECT * FROM fmcg_services_page 
...
exports.create = async (req, res) => {
    try {
        ...
        const [existing] = await db.query(
            `SELECT id FROM fmcg_services WHERE slug = ?`,
            [slug]
        );
        ...
        const image = req.file
            ? `/uploads/fmcg-services/${req.file.filename}`
            : null;

        await db.query(
            `INSERT INTO fmcg_services
...
exports.update = async (req, res) => {
    try {
        ...
        const [rows] = await db.query(
            `SELECT * FROM fmcg_services WHERE id = ?`,
            [id]
        );
        ...
        if (title_en && title_en !== service.title_en) {
            ...
            const [duplicate] = await db.query(
                `SELECT id FROM fmcg_services WHERE slug = ? AND id != ?`,
                [slug, id]
            );
        ...
        if (req.file) {
            ...
            image = `/uploads/fmcg-services/${req.file.filename}`;
        }

        await db.query(
            `UPDATE fmcg_services SET
                title_en = ?,
                title_ar = ?,
                description_en = ?,
                description_ar = ?,
...
exports.delete = async (req, res) => {
    try {
        ...
        const [rows] = await db.query(
            `SELECT image FROM fmcg_services WHERE id = ?`,
            [id]
        );
        ...
        await db.query(
            `DELETE FROM fmcg_services WHERE id = ?`,
            [id]
        );
